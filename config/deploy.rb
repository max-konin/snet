...
set :application, "snet"
set :site_domain, "snet.kerweb.ru"
...
set :scm, :git
set :repository,  "https://github.com/max-konin/snet.git"
...
namespace :deploy do
  task :restart do
    run "if [ -f #{unicorn_pid} ] && [ -e /proc/$(cat #{unicorn_pid}) ]; then kill -USR2 `cat #{unicorn_pid}`; else 
cd #{deploy_to}/current && bundle exec unicorn -c #{unicorn_conf} -E #{rails_env} -D; fi"
  end
  task :start do
